...
CREATE TABLE IF NOT EXISTS `r_dataset_organization` (
  ...
  PRIMARY KEY (`id`),
  CONSTRAINT `FK_DatasetID_r_dataset_organization` FOREIGN KEY (`dataset_id`) REFERENCES `t_dataset` (`id`) ON DELETE CASCADE,
  CONSTRAINT `FK_OrganID_r_dataset_organization` FOREIGN KEY (`organization_id`) REFERENCES `t_organization` (`id`) ON DELETE CASCADE
